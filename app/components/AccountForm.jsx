            ...
            key={index}
             className="flex items-center justify-between border-b p-2">
                ...
                <button className={`ml-2 text-sm 
                    ${accounts.length > 0 && !disabled? `hover:text-red-600 text-red-500`: ` text-gray-500`}`}
                ...
                <button className={`ml-2 text-sm 
                    ${account.name && !disabled? `hover:text-green-600 text-green-500`: ` text-gray-500`}`}
                    onClick={()=>addAccount(account.name)} disabled={disabled}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-3">
